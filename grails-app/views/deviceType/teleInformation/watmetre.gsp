...
<body onload="onLoadDeviceChart();">
	<g:applyLayout name="applicationHeader">
		<h3>
		<g:link style="color:black;" action="edit" controller="device" id="${ device.id }"> ${ device.label } (${ device.value })</g:link>
		<span class="h6">${ app.formatUserDateTime(date: device.dateValue) } - Il y a ${ app.formatTimeAgo(date: device.dateValue) }</span>
		</h3>
	</g:applyLayout>
