...
class HQL {
	...
	HQL(String selectPart, String fromPart) {
		if (selectPart) {
	...
	HQL(String selectPart, String fromPart, String selectCountPart) {
	...
	HQL addCriterionIds(String property, String ids, String prefixParam, Class idType = Long) {
		if (ids.contains("-")) {
			...
			def tokenIds = ids.split(",").toList().collect {
				it.trim().asType(idType)
			}
